...
import { getGameState, setGameState } from '../utils/kv';
import { createAsyncDispatch } from '../middleware/asyncDispatch';
...
const gameReducer = async (state: GameState, action: GameAction): Promise<GameState> => {
  switch (action.type) {
    case 'JOIN_GAME': {
      ...
      try {
        const gameState = await getGameState(codeFromUrl);
        if (!gameState) {
        ...
        await setGameState(codeFromUrl, newState);
        return newState;
      } catch (error) {
    ...
    case 'START_NEW_ROUND': {
      ...
      await setGameState(gameCode, newState);

      // Add a small delay to ensure all clients receive the update
      setTimeout(() => {
        const storedState = getGameState(gameCode);
        if (storedState) {
    ...
    case 'START_NEW_EPOCH': {
      ...
      await setGameState(state.gameCode, newState);
      return newState;
    }
...
export const GameProvider = ({ children }: { children: ReactNode }) => {
  ...
  const asyncDispatch = useMemo(
    () => createAsyncDispatch(dispatch, () => state),
    [dispatch]
  );
  ...
  useEffect(() => {
    ...
    if (codeFromUrl) {
      const fetchInitialState = async () => {
        try {
          const storedState = await getGameState(codeFromUrl);
          if (storedState) {
            asyncDispatch({ 
              type: 'SET_INITIAL_STATE', 
              payload: Promise.resolve(storedState)
            });
          }
        } catch (error) {
  ...
  return (
    <GameContext.Provider value={{ state, dispatch: asyncDispatch }}>
      {children}
